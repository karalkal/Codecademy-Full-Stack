...
export const RestaurantsReducer = (state = RestaurantsInitialState, action) => {
  switch (action.type) {
    ...
    case "STAR_RESTAURANT": {
      // stores new entry into state but we need to fetch from DB actually (as we need name too), this does not work
      // workaround is to store the name in BE instead and this is what I will do
      return {
        ...state,
